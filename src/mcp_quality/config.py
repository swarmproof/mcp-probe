...
A single :class:`ProbeConfig` is threaded through :class:`~mcp_quality.models.ProbeContext`
...
DEFAULT_CONFIG_FILENAMES = (".mcp-quality.toml", "mcp-quality.toml")
...
class ProbeConfig:
    ...
    snapshot_path: str = ".mcp-quality/snapshot.json"
    cache_dir: str = ".mcp-quality/cache"
...
def _from_file(start: Path) -> dict[str, Any]:
    """Load the nearest ``.mcp-quality.toml`` walking up from ``start`` (repo-root friendly)."""
    ...
    for directory in (start, *start.parents):
        for filename in DEFAULT_CONFIG_FILENAMES:
            ...
            if candidate.is_file():
                ...
                # accept either a flat table or a [tool.mcp-quality] section
                section = data.get("tool", {}).get("mcp-quality", data)
