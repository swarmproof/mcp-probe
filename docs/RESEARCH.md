...
| Tool | What it does | Engine / method | Relationship to mcp-quality |
|---|---|---|---|
...
**Governing rule:** mcp-quality leads with *quality*, names every one of these as a friend, and shells out to them for the hard 20% of security. It never markets itself as a scanner.

### 2.2 QUALITY lane — forming, unconsolidated (this is the wedge)

| Tool | What it covers | Critical gaps (mcp-quality's opening) |
|---|---|---|
...
| Origin domain | Prior art | What mcp-quality borrows | What it changes |
|---|---|---|---|
...
1. **Suite, not a score.** Five check families under one CI-native command with one `--fail-under` gate. mcp-xray gives a number; mcp-quality gives a *build status*. The analogy is **pytest** (a runner + a gate + an ecosystem), not a single linter.
...
3. **The gate + the badge + the trend.** `--fail-under B`, a README badge (`mcp-quality: A`), and historical score tracking (v0.2). This is the distribution flywheel; a manual X-ray has none of it.
4. **Cooperation as a feature.** `--deep-security` runs mcp-scan / Cisco and *folds their findings into one report* — mcp-quality is the aggregator/orchestrator of the ecosystem, positioned above the point tools rather than beside them. It can equally wrap mcp-xray's token engine if that proves the best implementation (buy-vs-build in ARCHITECTURE).
5. **The upgrade path nobody can copy without the portfolio.** `stampede --from-probe` promotes a static probe target into a full behavioral swarm simulation. mcp-quality is the on-ramp to an entire trust-infrastructure toolkit (§6).
...
|---|---|---|---|---|---|
| **P1** | **MCP server builder** (primary) | "Gate my releases on MCP quality like I gate on tests and lint." | Pre-commit / PR / release | All five; Contract+Cost+Perf on the zero-LLM fast path | mcp-quality in `.github/workflows/`; badge in README |
...
| **P6** ⊕ | **Framework maintainer** (FastMCP, SDK authors) | "Ship an official mcp-quality check so our users get a score by default." | Framework release | All five | Official adapter / template shipped |

---

## 6. Ecosystem context — the Swarm Proof toolkit (fit + handoffs)

mcp-quality is **#3, the wedge** — smallest real tool, hottest ecosystem, ships **first** of the code tools (after the two presence repos). It is the **static/CI little sibling of stampede**.

**Shared primitives reused (vendor-first per portfolio decision, extract at stampede v0.2):**

| Primitive | Home (first build) | mcp-quality usage |
|---|---|---|
...
**The `stampede --from-probe` handoff (the flagship link).** stampede's `MCPTarget` adapter interface is `discover() -> ToolSet`, `invoke(tool, args, agent_ctx) -> Result`, `reset()`, driven by a committed `stampede.yaml`. mcp-quality already performs connect + discover and knows the transport/command. So a probe run can **emit a `stampede.yaml` seed + discovered ToolSet + trace baseline**, and `stampede --from-probe ./mcp-quality-report.json` boots a full behavioral simulation of the exact server mcp-quality just graded — "your server scored a B; now watch 200 agents actually use it." Contract in ARCHITECTURE §9.

**Cross-tool synergy:** mcp-quality's Cost engine and costbomb's denial-of-wallet fuzzing share the token-accounting substrate; a probe Cost finding ("this tool's output is unbounded") is a costbomb seed. mcp-quality emits trace-format that agent-postmortems can reference.
...
| Gap | Evidence | mcp-quality response | Family |
|---|---|---|---|
...
5. **Registry API trust.** If a registry scores servers via mcp-quality, how do we prevent gaming (e.g., a server that detects the probe and behaves differently)? Signed/randomized probe goals?
