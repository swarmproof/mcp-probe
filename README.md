...
**▶ See the [live demo](./docs/demo.md)** — the token tax and the disambiguation matrix, captured from real runs. Or the [leaderboard](./docs/leaderboard.md): mcp-quality run against real public MCP servers.
...
fold in mcp-scan / Cisco findings. Commit a baseline with `mcp-quality snapshot "…"` so
...
Outputs a terminal report, JSON for CI, and an **`mcp-quality: A`** badge for your README. See [`SPEC.md`](./SPEC.md) and [`ROADMAP.md`](./ROADMAP.md).
...
| **mcp-quality** ← *you are here* | The CI quality suite for MCP servers — lint, contract-test, benchmark, load |
